...
// If Clerk keys are missing, skip middleware to avoid 500 errors.
const hasClerkKeys =
  !!process.env.CLERK_PUBLISHABLE_KEY && !!process.env.CLERK_SECRET_KEY;
if (!hasClerkKeys) {
  console.warn(
    "⚠️ Clerk keys are missing. Skipping Clerk middleware. Set CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY in backend/.env to enable authentication."
  );
} else {
